...
我們會發現因為 $A$ 一定大於 $0$，所以 $\gamma$ 一定小於 $1$。並且 $\gamma$ 跟 $a$ 無關只跟 $\beta$（或 $A$）有關。也就是說摩擦力會造成我們實際的收益的下降，並且對數報酬率的下降，正好等於原始資金與存入前總資金的比值。
