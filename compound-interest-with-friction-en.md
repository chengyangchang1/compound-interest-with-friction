...
But now, assuming that there is a fee for withdrawing and redepositing the revenue, we make the amount of funds after the redeposit $\beta$ times the original amount. **Assuming that the handling fee for withdrawals and re-deposits is proportional to the current amount of funds, since $F \propto V_f$, $\beta$ is independent of $V_f$, and we can see that it is a constant value of $\beta$ at this point. ** Then each time you make a deposit after the handling fee is charged, it becomes:
