...
We then look at the property of this solution, and we see that this solution gives the same result if we change the unit of time, for example, if we replace the day with the week as the unit. We know that $A = \displaystyle \frac{\displaystyle a }{\displaystyle n}$. Since we are splitting $n$ times per unit of time, $\displaystyle \frac{\displaystyle 1 }{\displaystyle n}$ is the optimal splitting period, and we get $\displaystyle \frac{\displaystyle 1}{\displaystyle n} = \frac{A}{a}$. Since $A$ is determined when $\beta$ is determined, $A$ divided by $a$ is the optimal split period. If we use day as the unit, then $a$ is the interest received in a day, and if we use week as the unit, then $a$ is the interest received in a week. Of course, if we use week as the unit, $a$ will become $7$ times bigger, and $\displaystyle \frac{\displaystyle 1 }{\displaystyle n}$, which is the optimal split period, will also be $7$ times smaller, but don't forget that the unit of time is changed from day to week, so the time that actually passes is unchanged, which means that the solution of this formula will not be changed because we change the unit of time. That is to say, the solution of this formula will not change the time interval of the optimal deposit cycle just because we change the unit of time. This is very reasonable, because the unit of time is set by human, and we should not get different results just because we use a different unit.
