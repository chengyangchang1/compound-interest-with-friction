...
Why don't we use $a$ and $n$ instead of $A$? Because interest rates fluctuate on-chain, so the $n$ calculated when interest rates fluctuate will also fluctuate, which in turn affects the optimal deposit period we get, and makes the deposit period fluctuate as well. However, we will find that $a$ always appears with $n$ (always divides), so $A$ is only related to $\beta$ and independent of $a$. Therefore, by using $A$, the effect of interest rate fluctuations can be ignored. Given $\beta$, $A$ can be calculated, and as long as the ratio of accumulated interest income to the original deposit reaches $A$, it is time to re-deposit.
