...
We can imagine that when there is a commission, if the operation is too frequent, the commission will erode the profitability and lead to a decrease in revenue, and if the operation period is too long, it will weaken the effect of compounding, which will also lead to a decrease in revenue. If we want to know what $n$ to maximise revenue. We can differentiate $n$ such that the extreme point is where the differential is equal to $0$. We let $\displaystyle \beta \left ( 1+\frac{a }{\displaystyle n} \right ) = g$, then the differential of the above equation $(1)$ becomes the differential of $g^{\displaystyle n}$. Since we are only asking for the value of $n$ when the differential is equal to $0$, and the logarithmic function is an increasing function, the $n$ that is equal to $0$ after taking the logarithm is also the $n$ that is equal to $0$ after differentiating the original function. So we can get:
